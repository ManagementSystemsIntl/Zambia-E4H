...
varlabs <- read_xlsx("data/July-Sep 2022/Child health/Child Health (Jan 2018 - Oct 2022).xlsx",
                  sheet="export",
                  range='A1:U1',
                  col_names = F) %>%
  t()
...
chp <- read_excel("data/July-Sep 2022/Child health/Child health monthly prov 2022.xlsx",
                  sheet="export",
                  range="A2:P132") %>%
...
chp_21 <- chp %>%
  filter(year==2021) %>%
  mutate(date=as.Date("2021-12-01"))
