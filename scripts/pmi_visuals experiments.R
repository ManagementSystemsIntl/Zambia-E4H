...
dt <- as.Date("2014-01-01")
dt1 <- as.Date("2014-02-01")
...
dat2_plt <- ggplot(dat2, aes(x=monthyr, y=value)) +
  ...
  labs(title="Nchelenge Trend Monthly Confirmed Cases",
       ...
       y="Cases") +
  base +
  geom_vline(xintercept = c(dt, dt1) ,color=c("#212721","#EF7D00") ,lty=c("solid", "solid") ,size=c(5,5), alpha=0.5)+
  annotate("text", x = dt, y = 0, label = "SBC", size=2, angle=90, hjust =-10) +
  annotate("text", x = dt1, y = 0, label = "Mass ITNS Distribution", size=2, angle=90, hjust =-2)
