...
ggplot(frthPlusANC_prov, aes(x=mnthyr, y=frth.ancP)) + 
  ...
  scale_y_continuous(limits = c(0,.5),
                     ...
                     breaks = c(.1,.2,.3,.4,.5)) +
  labs(x ="", y="", caption = "Data Source: HMIS") +labs(x ="", y="", caption = "Data Source: HMIS") +
  ggtitle("4th+ to Total ANC attendances paints a similar picture to the national one,\n where there is a general decline begining 2022, (Oct 2019 - Sept 2024).") +
  facet_wrap(~prov, ncol=4) +
  faceted +
  scale_color_manual(values=usaid_blue) + basey

ggsave("viz/Ad hoc Jun 2024/4th+ to Total ANC attendances facets.png",
       device="png",
       type="cairo",
       height = 6.5,
       width = 12)
...
ggplot(frthPlusANC, aes(x=mnthyr, y=frth.ancP)) + 
  ...
  labs(x ="", y="", caption = "Data Source: HMIS") +labs(x ="", y="", caption = "Data Source: HMIS") +
  ggtitle("The Trends for the 4th+ to Total ANC attendances has been below 30% at National Level\n and begining mid-2022 it is on a downward trend (Oct 2019 - Sept 2024).") +
  scale_color_manual(values=usaid_blue) + basey
...
frthPlusANC_prov <- read_xls("data/June 2024 Ad Hoc/4th+ ANC Western districts.xls")
names(frthPlusANC_prov)
frthPlusANC_prov
frthPlusANC_prov  <- frthPlusANC_prov  %>%
  mutate(month_chr = str_sub(periodname,
...
distMMR <- read_xls("data/June 2024 Ad Hoc/Maternal MR_Luapula districts.xls")
names(distMMR)
distMMR
distMMR  <- distMMR  %>%
  mutate(month_chr = str_sub(periodname,
...
ggplot(distMMR, aes(x=mnthyr, y=matmortratio)) + 
  ...
  scale_y_continuous(labels = comma) +
  labs(x ="", y="", caption = "Data Source: HMIS") +labs(x ="", y="", caption = "Data Source: HMIS") +
  ggtitle("Maternal Mortality Ratio per 100, 000 live births, Luapula Province Districts (Oct 2019 - Sept 2024).") +
  facet_wrap(~dist, ncol=4) +
  faceted +
  scale_color_manual(values=usaid_red,
                     labels ="Maternal Mortality Ratio Per 100, 000 Deliveries.") + basey

ggsave("viz/Ad hoc Jun 2024/MMRatios_Luapula districts.png",
       device="png",
       type="cairo",
       height = 6.5,
       width = 12.5)
...
prov_mmr_plt <- ggplot(mmr_1, aes(x = mnthyr, y = deaths , colour =   mmtype, linetype=mmtype)) + 
  ...
  scale_linetype_manual(name="",
                        labels= ("Maternal mortality facility ratio (per 100,000 live births)"), 
                        values=("solid"))+
  labs(x="", y="", caption="Data Source: HMIS", title="Maternal mortality facility ratio (per 100,000 live births) show unique patterns in Northern, and Western\n provinces starting mid 2022 as the other provinces show an onset of a decline in MMR (Jan 2020 - Sept 2024).") +
  scale_color_manual(name ="",
                     values = usaid_red) + 
  baseX
...
#'
#'
district_neonat <- read_xls("data/Nov 2024 MHDR/Neonatal death counts_Eastern districts.xls")
...
Dist_ndc_plt <- ggplot(ndcount_1, aes(x = mnthyr, y = deaths , colour =   mmtype, linetype=mmtype)) + 
  ...
  scale_linetype_manual(name="",
                        labels= ("Neonatal Deaths Count (absolute numbers"), 
                        values=("solid"))+
  labs(x="", y="", caption="Data Source: HMIS", title="The Neonatal deaths Count (absolute numbers) for Eastern province districts (Oct 2019 - Sept 2024).") +
  scale_color_manual(name ="",
                     values = usaid_red) + 
  baseX

Dist_ndc_plt
ggsave("viz/Nov 2024 FHDR/Neonatal death count_Eastern districts.png",
       device="png",
       type="cairo",
       height = 6.5,
       width = 12)
