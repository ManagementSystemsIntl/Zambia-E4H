...
distMMR <- read_xls("data/June 2024 Ad Hoc/Maternal MR_Southern districts.xls")
names(distMMR)
distMMR
distMMR  <- distMMR  %>%
  mutate(month_chr = str_sub(periodname,
...
ggplot(distMMR, aes(x=mnthyr, y=matmortratio)) + 
  ...
  scale_y_continuous(labels = comma) +
  labs(x ="", y="", caption = "Data Source: HMIS") +labs(x ="", y="", caption = "Data Source: HMIS") +
  ggtitle("Maternal Mortality Ratio per 100, 000 live births, Southern Province Districts (Oct 2019 - Sept 2024).") +
  facet_wrap(~dist, ncol=4) +
  faceted +
  scale_color_manual(values=usaid_red,
                     labels ="Maternal Mortality Ratio Per 100, 000 Deliveries.") + basey

ggsave("viz/Ad hoc Jun 2024/MMRatios_Southern districts.png",
       device="png",
       type="cairo",
       height = 6.5,
       width = 12.5)
