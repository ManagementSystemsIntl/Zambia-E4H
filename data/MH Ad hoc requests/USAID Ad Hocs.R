...
frthPlusANC_prov <- read_xls("data/June 2024 Ad Hoc/4th+ ANC Luapula districts.xls")
names(frthPlusANC_prov)
frthPlusANC_prov
frthPlusANC_prov  <- frthPlusANC_prov  %>%
  mutate(month_chr = str_sub(periodname,
...
ggplot(frthPlusANC_prov, aes(x=mnthyr, y=frth.ancP)) + 
  ...
  scale_y_continuous(limits = c(0,.5),
                     ...
                     breaks = c(.1,.2,.3,.4,.5)) +
  labs(x ="", y="", caption = "Data Source: HMIS") +labs(x ="", y="", caption = "Data Source: HMIS") +
  ggtitle("The 4th+ ANC attendances for Luapula Province districts, have unique trends in Chembe, Chipili, and\n Mwansabombwe. Whereas the other district give a similar gardual decline in performance (Oct 2019 - Sept 2024).") +
  facet_wrap(~prov, ncol=4) +
  faceted +
  scale_color_manual(values=usaid_blue) + basey

ggsave("viz/Ad hoc Jun 2024/4th+ ANC visits_Luapula districts.png",
       device="png",
       type="cairo",
       height = 6.5,
       width = 12.5)
