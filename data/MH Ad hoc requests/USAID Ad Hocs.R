...
#'
#'
district_neonat <- read_xls("data/Nov 2024 MHDR/Neonatal death counts_Lusaka districts.xls")
...
Dist_ndc_plt <- ggplot(ndcount_1, aes(x = mnthyr, y = deaths , colour =   mmtype, linetype=mmtype)) + 
  ...
  scale_linetype_manual(name="",
                        labels= ("Neonatal Deaths Count (absolute numbers"), 
                        values=("solid"))+
  labs(x="", y="", caption="Data Source: HMIS", title="The Neonatal deaths Count (absolute numbers) for Lusaka province districts (Oct 2019 - Sept 2024).") +
  scale_color_manual(name ="",
                     values = usaid_red) + 
  baseX

Dist_ndc_plt
ggsave("viz/Nov 2024 FHDR/Neonatal death count_Lusaka districts.png",
       device="png",
       type="cairo",
       height = 6.5,
       width = 12)
